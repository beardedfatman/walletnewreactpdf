  ...
  try {
    ...
    /* Calculate total assets and total liabilities */
	  ...
	  const totalSTLiabilities =
		...
		//alert(totalSTLiabilities);
	 /* end Total assets and total liabilities */
	
   // const totalAssets = 5000
    const totalLiabilities =totalLTLiabilities+ totalSTLiabilities;
  //  alert(totalLiabilities+''+totalAssets);
    ...
    const drawAssetAllocationPage = () => {
      ...
      const equities = parseFloat(netWorth.valueOfStocks || 0);
    ...
    const drawDebt = () => {
      ...
      const sumOfAllLoans = Number(loanData.mortgage) + Number(loanData.remainingHouseLoan || 0) + Number(loanData.remainingVehicleLoan) + Number(loanData.remainingStudyLoan) + Number(loanData.remainingRenovationLoan) + Number(loanData.remainingCreditCardsLoan) + Number(loanData.remainingPersonalDebitLoan);
      ...
      const debtService = Number(sumOfAllLoans || 0) / Number(income || 0);
     ...
     // const solvencyRatio = netWorthAmount / (totalAssets - totalCPF)
      const solvencyRatio = netWorthAmount / (totalAssets)
		console.log("totalAssets",totalAssets,sumOfAllLoans)
      ...
      const totalInvestment = Number(netWorth.valueOfCommodities) + Number(netWorth.valueOfOtherInvestments) + Number(netWorth.valueOfCryptocurrencies) + Number(netWorth.valueOfProperties) + Number(netWorth.valueOfInsurancePolicies) + Number(netWorth.valueOfUnitTrusts) + Number(netWorth.valueOfBonds) + Number(netWorth.valueOfStocks);
      const totalNetworth = Number(netWorth.currentSavings) + Number(netWorth.currentValueOfFixedDeposits) + (Number(netWorth.currentValueOfOtherCPF || 0)) + (Number(netWorth.currentValueOfMedisaveCPF || 0)) + (Number(netWorth.currentValueOfSpecialCPF || 0)) + (Number(netWorth.currentValueOfOrdinaryCPF || 0)) + Number(netWorth.valueOfCommodities) + Number(netWorth.valueOfOtherInvestments) + Number(netWorth.valueOfCryptocurrencies) + Number(netWorth.valueOfProperties) + Number(netWorth.valueOfInsurancePolicies) + Number(netWorth.valueOfUnitTrusts) + Number(netWorth.valueOfBonds) + Number(netWorth.valueOfStocks);
    ...
    const drawWill = (page, willData, font, boldFont, fontSize = 12) => {
      ...
      y = drawText("2. I declare that in the subsequent clauses of this my Will the expression “my Trustee” shall (where the", x, y);
      ...
      y = drawText(``, x, y)
      y = drawText("3. Subject to the payment of my just debts funeral and testamentary expenses, I give, devise and ", x, y);
      ...
      beneficiaries.forEach((beneficiary, index) => {
        y = drawText(`3.${index + 1} to give ${beneficiary.percentOfAsset}% to ${beneficiary.name} ${beneficiary.nric}`, x, y);
      });
      y = drawText("3.3 to give the rest of my property to my children equally", x, y);
      y = drawText(``, x, y)

      // Additional static content
      y = drawText("4. My Trustees shall have the following powers:", x, y);
      y = drawText("4.1 to apply the whole or such part as my Trustees shall think fit of the expectant or presumptive share", x, y);
      ...
      y = drawText("4.2 to apply the whole or such part as my Trustees shall think fit of the presumptive contingent expectant", x, y);
      ...
      y = drawText("4.3 to make payments to the parent or guardian or guardians of any beneficiary under this Will without", x, y);
    ...
    const drawWitnessSection = (page, font, boldFont, fontSize = 12) => {
      ...
      y = drawText("5. My Trustees shall be entitled to be indemnified out of my assets of any residuary estate against all", x, y);
	...
	var actualamountneed=[];
     const drawInvestmentsRec = () => {
      ...
      const StargetYear = new Date(goals.shortTermGoals[0].targetDate).getFullYear()
      const sYearsToReach = StargetYear - currentYear;
	  
	  const MtargetYear = new Date(goals.midTermGoals[0].targetDate).getFullYear()
      const MYearsToReach = MtargetYear - currentYear;
	  
	  const LtargetYear = new Date(goals.longTermGoals[0].targetDate).getFullYear()
      const LYearsToReach = LtargetYear - currentYear;
    ...
    function calculateFutureValue(rate, nper, pmt, pv = 0, type = 0) {
      ...
      const shortTermGoalsTotal = calculateFutureValue(shortTermRate, sYearsToReach, -Number(surplusInflow), -Number(surplusSaving))
	  ...
	  const midTermGoalsTotal = calculateFutureValue(midTermRate, (MYearsToReach-sYearsToReach), -Number(surplusInflow), -Number(B25))+ calculateFutureValue(midTermRate, MYearsToReach, 0, -Number(netWorth.valueOfBonds))

	
	  const midTermGoalsInflationAdjusted = calculateFutureValue(0.03, MYearsToReach, 0, -(Number(goals.midTermGoals[0].amount)));
	
      const D28=(parseFloat(midTermGoalsTotal-midTermGoalsInflationAdjusted)>0) ? parseFloat(midTermGoalsTotal-midTermGoalsInflationAdjusted) : 0;
